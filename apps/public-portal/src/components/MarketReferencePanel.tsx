...
export function MarketReferencePanel({
  ...
  const handleAdd = () => {
    ...
    // Normalize common user date inputs (e.g. "20/08/26" or "20/08/2026")
    let normalizedDate = draft.observationDate.trim();
    const ddmmyy = /^(\d{1,2})\/(\d{1,2})\/(\d{2})$/; // 20/08/26
    const ddmmyyyy = /^(\d{1,2})\/(\d{1,2})\/(\d{4})$/; // 20/08/2026
    if (ddmmyyyy.test(normalizedDate)) {
      const parts = normalizedDate.match(ddmmyyyy)!.slice(1);
      const dd = parts[0].padStart(2, "0");
      const mm = parts[1].padStart(2, "0");
      const yyyy = parts[2];
      normalizedDate = `${yyyy}-${mm}-${dd}`;
    } else if (ddmmyy.test(normalizedDate)) {
      const parts = normalizedDate.match(ddmmyy)!.slice(1);
      const dd = parts[0].padStart(2, "0");
      const mm = parts[1].padStart(2, "0");
      const yy = parts[2];
    ...
    try {
      ...
      console.error("Failed to add comparable listing", err);
      setHasError(true);
      return;
    }
    setDraft({
        ...
        <div className="comparable-form">
          ...
          <input
            ...
            placeholder={fieldText(
              "Observation date (YYYY-MM-DD or DD/MM/YY) *",
              "የመረጃው ቀን (YYYY-MM-DD ወይም DD/MM/YY) *"
            )}
